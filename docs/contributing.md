...
- `uv run --no-project --python 3.11 --with-requirements requirements-dev.txt python -m pytest -c pytest.ini tests/ Orchestrator/tests/ -q`: **337 passed** (**154 root + 183 Orchestrator**, including **24** local-endpoint tests). All **13** standalone Node regressions passed; frontend lint, strict `tsc --noEmit`, and the normal build passed.
