...
It runs the root and Orchestrator pytest suites, all fifteen standalone Node regressions (including runtime /object_info schema discovery, mounted production workflow-editor/parameter-panel controls, session storage, and production recovery-consumer checks), frontend lint, and the production build. It never adds `PYTHONPATH`; `-c pytest.ini` makes the root import paths explicit when both test trees are selected. The command intentionally fails if any check fails.
...
- `uv run --no-project --python 3.11 --with-requirements requirements-dev.txt python -m pytest -c pytest.ini tests/ Orchestrator/tests/ -q`: **337 passed** (**154 root + 183 Orchestrator**, including **24** local-endpoint tests). All **15** standalone Node regressions passed; frontend lint, strict `tsc --noEmit`, and the normal build passed.
