...
class RuleEngine:
    ...
    def apply_live_action_preset(
        ...
        # Dynamic shot_size mapping - includes ALL enum values automatically
        shot_size_map = {s.value: s for s in ShotSize}
        # Add space-separated and alternative name aliases for backward compatibility
        shot_size_map.update({
            "Extreme Wide Shot": ShotSize.EWS, "Extreme_Wide_Shot": ShotSize.EWS,
            "Wide Shot": ShotSize.WS, "Wide_Shot": ShotSize.WS,
            "Medium Wide Shot": ShotSize.MWS, "Medium_Wide_Shot": ShotSize.MWS,
            "Medium Shot": ShotSize.MS, "Medium_Shot": ShotSize.MS,
            "Medium Close Up": ShotSize.MCU, "Medium_Close_Up": ShotSize.MCU,
            "Close Up": ShotSize.CU, "Close_Up": ShotSize.CU,
            "Big Close Up": ShotSize.BCU, "Big_Close_Up": ShotSize.BCU,
            "Extreme Close Up": ShotSize.ECU, "Extreme_Close_Up": ShotSize.ECU,
            "Over The Shoulder": ShotSize.OTS, "Over_The_Shoulder": ShotSize.OTS,
            "Point_of_View": ShotSize.POV,
            "Cowboy_Shot": ShotSize.AMERICAN,
        })
        
        # Dynamic composition mapping - includes ALL enum values automatically
        # This prevents "Unknown composition" warnings when presets use valid enum values
        composition_map = {c.value: c for c in Composition}
        # Add space-separated aliases for backward compatibility
        composition_map.update({
            ...
            "Frame Within Frame": Composition.FRAME_WITHIN_FRAME,
            "Golden Ratio": Composition.GOLDEN_RATIO,
        })
        
        mood_map = {m.value: m for m in Mood}  # Direct value mapping
        
        # Dynamic color_tone mapping - includes ALL enum values automatically
        color_tone_map = {ct.value: ct for ct in ColorTone}
        # Add short aliases for backward compatibility
        color_tone_map.update({
            "Warm": ColorTone.WARM_SATURATED,
            "Cool": ColorTone.COOL_SATURATED,
            "Neutral": ColorTone.NEUTRAL_SATURATED,
            "Teal Orange": ColorTone.TEAL_ORANGE,
            "Highly_Saturated": ColorTone.WARM_SATURATED,  # Fallback alias
            "Muted": ColorTone.NEUTRAL_DESATURATED,  # Fallback alias
        })
        
        warnings: list[ValidationMessage] = []
        
        # Helper to safely map values with warnings
        ...
        # Dynamic lighting style mapping - includes ALL enum values automatically
        lighting_style_map = {ls.value: ls for ls in LightingStyle}
        
        # Dynamic lighting source mapping - includes ALL enum values automatically
        lighting_source_map = {ls.value: ls for ls in LightingSource}
