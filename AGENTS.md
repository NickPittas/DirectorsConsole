...
1. **CinemaPromptEngineering (CPE)** - Cinematic rules engine for camera, lighting, and era-specific prompt generation
   ...
   - **⚠️ NOTE:** `cinema_rules/` is authoritative; the standalone node's copy is generated by `scripts/sync_comfy_node.py` and must not be edited directly
...
- **Packaging**: Poetry (CPE) and Hatchling (Orchestrator)
...
│       ├── api_server_main.py       # Full API entry point
│       ├── server.py                # CLI API server wrapper
│       ├── core/                   # Core engine
...
npm run build:comfyui    # Build to ComfyCinemaPrompting/web/app/
npm run lint             # ESLint check
```

**Build outputs:**
- The CPE API serves optional static assets from `CinemaPromptEngineering/static/`.
- The ComfyUI node frontend is served from `CinemaPromptEngineering/ComfyCinemaPrompting/web/app/`.
- Frontend output mode is configured in `CinemaPromptEngineering/frontend/vite.config.ts`.
...
**Provider configuration:** The authoritative Google AI (Gemini) API-key and Antigravity OAuth instructions are in the [README provider setup](README.md#ai-llm-provider-setup). The `google` provider uses a Google AI Studio API key; it is not the Antigravity OAuth path. Antigravity app configuration belongs in the private, gitignored `CinemaPromptEngineering/.env` (mode `0600` on Unix), using the tracked `.env.example` only when `.env` does not already exist. Fill `ANTIGRAVITY_CLIENT_ID` and `ANTIGRAVITY_CLIENT_SECRET`, restart the CPE backend, and never put them in frontend `.env`/`VITE_*` variables, Git, logs, or issues. The loader is implemented and fresh-process configuration checks pass; live provider/account verification remains pending.
...
3. The API serves static assets from `CinemaPromptEngineering/static/`; the separate ComfyUI node build uses `CinemaPromptEngineering/ComfyCinemaPrompting/web/app/`

---

## Security Considerations

1. **Credential Storage**: Use `api/providers/credential_storage.py` for backend credential storage, and protect browser profiles/local data because provider configuration may also be cached there.
2. **Private deployment**: This project has no application-wide authentication or authorization. Reachable peers can invoke exposed functions and submit malicious input; do not expose services to public or untrusted networks.
3. **Network responsibility**: Host/network administrators own access controls, firewalls, VPNs, and segmentation. Isolation does not guarantee protection from malicious files, workflows, or vulnerabilities.
4. **OAuth app configuration**: Antigravity client configuration is external; see the [README provider setup](README.md#antigravity-oauth-app-configuration). Existing tokens are not deleted by configuration cleanup, and re-login alone cannot fix missing app configuration.
5. **Path Traversal**: Always validate paths when accessing user-provided file paths.
6. **Audit status**: This is not a formal security audit or a hardened public-service deployment. Documentation changes do not alter network policy.
...
- Canonical `cinema_rules/` plus the generated standalone node copy require the sync helper to stay aligned
