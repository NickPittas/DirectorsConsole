        ...
        try:
            ...
            if sys.platform == "win32":
                ...
                if str(old_pid) in result.stdout and "python" in result.stdout.lower():
                    print(f"\033[91mERROR: start.py is already running (PID {old_pid})\033[0m")
...
def log(prefix: str, message: str, prefix_color: str = Colors.WHITE, msg_color: str = Colors.GRAY) -> None:
...
class EnvironmentManager:
    ...
    def _setup_environments(self) -> None:
        ...
        self.environments.append(EnvironmentConfig(
            name="orchestrator",
            display_name="Orchestrator",
            working_dir=orch_dir,
            venv_path=orch_dir / ".venv",
            requirements_file=orch_dir / "requirements.txt",
            required_imports=["fastapi", "uvicorn", "pydantic", "httpx", "loguru", "PIL"],
        ...
        self.environments.append(EnvironmentConfig(
            ...
            required_imports=["fastapi", "uvicorn", "pydantic", "httpx", "loguru", "PIL", "cryptography"],
    ...
    def create_venv(self, env: EnvironmentConfig) -> bool:
        ...
        log(env.name.upper(), f"Creating virtual environment at {env.venv_path}...", env.prefix_color)
        ...
        try:
            ...
            if result.returncode != 0:
                log(env.name.upper(), f"Failed to create venv: {result.stderr}", Colors.RED)
                return False
    ...
    def install_requirements(self, env: EnvironmentConfig) -> bool:
        ...
        if not env.requirements_file.exists():
            log(env.name.upper(), f"Requirements file not found: {env.requirements_file}", Colors.YELLOW)
            return True  # Not an error, just no requirements
        
        python_exe = self.get_python_exe(env.venv_path)
        use_uv = self.uv_path and self.is_uv_venv(env.venv_path)
        
        log(env.name.upper(), "Installing dependencies from requirements.txt...", env.prefix_color)
        
        try:
            if use_uv:
                # Use uv pip for uv-managed venvs
                log(env.name.upper(), "Using uv pip (uv-managed venv detected)", Colors.GRAY)
                result = subprocess.run(
                    [self.uv_path, "pip", "install", "-r", str(env.requirements_file),
                ...
                result = subprocess.run(
                    [str(python_exe), "-m", "pip", "install", "-r", str(env.requirements_file)],
                    capture_output=True,
                    text=True,
                    timeout=600
                )
    ...
    def install_missing_packages(self, env: EnvironmentConfig, packages: list[str]) -> bool:
        ...
        log(env.name.upper(), f"Installing missing packages: {', '.join(packages_to_install)}", env.prefix_color)
        
        try:
            if use_uv:
                # Use uv pip for uv-managed venvs
                result = subprocess.run(
                    [self.uv_path, "pip", "install", "--python", str(python_exe)] + packages_to_install,
                    capture_output=True,
                    text=True,
                    timeout=300
                )
            ...
            if result.returncode != 0:
                ...
                log(env.name.upper(), f"Failed to install packages: {error_msg}", Colors.RED)
                return False
    ...
    def setup_environment(self, env: EnvironmentConfig, force_reinstall: bool = False) -> bool:
        ...
        if not success:
            log(env.name.upper(), f"Missing imports: {', '.join(failed)}", Colors.YELLOW)
            ...
            success, failed = self.verify_imports(env)
            if not success:
                log(env.name.upper(), f"Still missing after install: {', '.join(failed)}", Colors.RED)
                log(env.name.upper(), "Try: --setup --force to reinstall all dependencies", Colors.YELLOW)
                return False
        ...
        if not success:
            ...
            if auto_fix:
                if self.install_missing_packages(env, failed):
                    success, failed = self.verify_imports(env)
                    if success:
                        log(env.name.upper(), "Fixed missing dependencies", Colors.GREEN)
                        return True
...
class ProcessManager:
    ...
    def _signal_handler(self, signum: int, frame) -> None:
        ...
        if not self.is_shutting_down:
            print()
            log("SHUTDOWN", "Received termination signal, stopping services...", Colors.YELLOW)
            self.cleanup_all()
            sys.exit(0)
    ...
    def start_process(self, config: ServiceConfig) -> Optional[subprocess.Popen]:
        ...
        try:
            ...
            log(config.prefix, f"Spawned PID {process.pid}: {' '.join(full_command[:4])}...", Colors.CYAN)
    ...
    def _windows_cleanup_orphans(self) -> None:
        ...
        try:
            ...
            result = subprocess.run(
                ["wmic", "process", "where", 
                 "commandline like '%uvicorn%'", "get", "processid"],
                capture_output=True,
                text=True,
                timeout=5
            )
...
def kill_orphaned_processes() -> None:
    ...
    try:
        ...
        result = subprocess.run(
            ["wmic", "process", "where", 
             "commandline like '%DirectorsConsole%'", "get", "processid,commandline"],
            capture_output=True,
            text=True,
            timeout=10
        )
        ...
        log("CLEANUP", f"Current PID: {current_pid}, Parent PID: {parent_pid}", Colors.GRAY)
                ...
                if f":{port}" in line and "LISTEN" in line:
                    ...
                    if parts:
                        ...
                        if pid.isdigit() and pid != "0" and pid not in pids_killed:
                            log("CLEANUP", f"Killing process {pid} on port {port}", Colors.YELLOW)
                            subprocess.run(
                                ["taskkill", "/F", "/PID", pid],
                                capture_output=True,
                                timeout=5
...
def stream_output(process: subprocess.Popen, prefix: str, prefix_color: str, 
                  stop_event: Optional[callable] = None) -> None:
...
def check_node() -> bool:
    ...
    if not node_path:
        log("UI", "Node.js not found! Please install from https://nodejs.org", Colors.RED)
        return False
...
def check_npm_modules(frontend_dir: Path) -> bool:
    ...
    print()
    response = input(color("  Install npm dependencies now? [Y/n]: ", Colors.WHITE)).strip().lower()
...
"""
    ...
    parser.add_argument("--no-orchestrator", action="store_true",
                        help="Skip starting Orchestrator")
    ...
    if args.setup:
        ...
        if check_node():
            ...
            check_npm_modules(frontend_dir)
    ...
    for port in ports_to_check:
        if is_port_in_use(port):
            log("CLEANUP", f"Port {port} is in use, attempting to free...", Colors.YELLOW)
            kill_process_on_port(port)
            
            # Verify it's free now
            if is_port_in_use(port):
                log("ERROR", f"Could not free port {port}. Please kill the process manually.", Colors.RED)
                sys.exit(1)
    ...
    if not args.no_orchestrator:
        ...
        orch_config = ServiceConfig(
            ...
            command=[
                ...
                "--host", "0.0.0.0",
                "--port", str(args.orchestrator_port),
            ],
        ...
        if wait_for_health(orch_config.health_endpoint, timeout=30, prefix="ORCH"):
            log("ORCH", f"Ready at http://localhost:{args.orchestrator_port}", Colors.GREEN)
        else:
    ...
    if not args.no_frontend:
        ...
        if not check_node():
            ...
            if not check_npm_modules(frontend_dir):
                ...
                env_content = f"""# Auto-generated by start.py
VITE_API_BASE=http://localhost:{args.backend_port}
VITE_ORCHESTRATOR_URL=http://localhost:{args.orchestrator_port}
"""
                ...
                try:
                    ...
                    while time.time() - start_time < 60:
                        if frontend_process.poll() is not None:
                            log("UI", "Frontend process exited unexpectedly", Colors.RED)
                            break
                        ...
                        try:
                            ...
                            if line:
                                line = line.strip()
                                if "ready in" in line.lower() or f"localhost:{args.frontend_port}" in line:
                                    vite_ready = True
                    ...
                    if vite_ready:
                        log("UI", f"Frontend ready at http://localhost:{args.frontend_port}", Colors.GREEN)
                    else:
    ...
    if not args.no_orchestrator:
        print(f"    {color('Orchestrator:', Colors.GRAY)}  {color(f'http://localhost:{args.orchestrator_port}', Colors.CYAN)}")
    print(f"    {color('CPE Backend:', Colors.GRAY)}   {color(f'http://localhost:{args.backend_port}', Colors.CYAN)}")
    print(f"    {color('API Docs:', Colors.GRAY)}      {color(f'http://localhost:{args.backend_port}/docs', Colors.CYAN)}")
    if not args.no_frontend and frontend_process:
        print(f"    {color('Frontend:', Colors.GRAY)}      {color(f'http://localhost:{args.frontend_port}', Colors.GREEN)}")
        ...
        while True:
            ...
            for name, proc in list(pm.processes.items()):
                if proc.poll() is not None:
                    log(name.upper(), f"Process exited with code {proc.returncode}", Colors.RED)
                    ...
                    try:
                        ...
                        if remaining:
                            for line in remaining.split("\n"):
                                if line.strip():
                                    log(name.upper(), line.strip(), Colors.RED, Colors.RED)
                    except:
                        pass
